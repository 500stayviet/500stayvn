...
exports.REAL_ESTATE_TERMS = void 0;
exports.translateRealEstateTerm = translateRealEstateTerm;
exports.applyRealEstateTerms = applyRealEstateTerms;
exports.getRealEstateTermsPrompt = getRealEstateTermsPrompt;
/**
 * 베트남어 -> 한국어 부동산 용어 사전
 */
exports.REAL_ESTATE_TERMS = [
    // 방/침실 관련
 ...
 * 베트남어 용어를 한국어로 변환
 */
function translateRealEstateTerm(viTerm) {
    const term = exports.REAL_ESTATE_TERMS.find(t => t.vi.toLowerCase() === viTerm.toLowerCase() ||
 ...
 */
function applyRealEstateTerms(text, targetLanguage = 'ko') {
    let translatedText = text;
    // 우선순위가 높은 긴 용어부터 매칭
    const sortedTerms = [...exports.REAL_ESTATE_TERMS].sort((a, b) => b.vi.length - a.vi.length);
 ...
 */
function getRealEstateTermsPrompt() {
    const termsList = exports.REAL_ESTATE_TERMS.map(term => `- "${term.vi}" → "${term.ko}"${term.en ? ` (${term.en})` : ''}`).join('\n');
    return `다음 부동산 용어 사전을 반드시 사용하여 번역하세요:\n\n${termsList}\n\n중요: 위 용어들은 정확히 표시된 한국어로 번역해야 합니다.`;
}
//# sourceMappingURL=translationTerms.js.map