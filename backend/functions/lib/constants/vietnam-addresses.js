...
exports.HCMC_DISTRICTS = exports.VIETNAM_PROVINCES = void 0;
...
function getProvinceByCode(code) {
    return exports.VIETNAM_PROVINCES.find(p => p.code === code);
}
/**
 * District 코드로 District 찾기
 */
function getDistrictByCode(code, provinceCode) {
    if (provinceCode) {
        return exports.HCMC_DISTRICTS.find(d => d.code === code && d.provinceCode === provinceCode);
    }
    return exports.HCMC_DISTRICTS.find(d => d.code === code);
}
/**
 * Province 코드로 Districts 찾기
 */
function getDistrictsByProvince(provinceCode) {
    return exports.HCMC_DISTRICTS.filter(d => d.provinceCode === provinceCode);
}
//# sourceMappingURL=vietnam-addresses.js.map