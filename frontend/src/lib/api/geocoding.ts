...
import { searchPlaceIndexForText, searchPlaceIndexForPosition, getLocationServiceLanguage } from './aws-location';
...
export async function geocodeAddress(
  ...
  try {
    ...
    const results = await searchPlaceIndexForText(address, locationLanguage);
...
export async function reverseGeocode(
  ...
  try {
    const locationLanguage = getLocationServiceLanguage(language);
    const results = await searchPlaceIndexForPosition(lat, lng, locationLanguage);
