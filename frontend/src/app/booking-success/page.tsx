...
import { CheckCircle, Calendar, MapPin, Clock, Users, MessageCircle, Home, Loader2, AlertCircle, ArrowLeft, Copy, Check } from 'lucide-react';
...
export default function BookingSuccessPage() {
  ...
  const [showSuccessModal, setShowSuccessModal] = useState(searchParams.get('new') === 'true');
  const [activeChatRoomId, setActiveChatRoomId] = useState<string | null>(null);
  const [copied, setCopied] = useState(false);

  useEffect(() => {
    // URL에 new=true가 없으면 성공 모달을 띄우지 않음
        ...
        <div className="p-4 space-y-4 pb-12">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
            <div className="bg-gray-50 px-4 py-3 border-b border-gray-100 flex items-center justify-between">
              <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">
                {currentLanguage === 'ko' ? '예약 번호' : 
                 currentLanguage === 'vi' ? 'Mã đặt phòng' : 
                 'Booking Number'}
              </span>
              <button 
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm space-y-5">
            <h3 className="text-sm font-bold text-gray-900 border-b border-gray-50 pb-3 -mx-1">
              {currentLanguage === 'ko' ? '상세 예약 정보' : 'Chi tiết đặt phòng'}
            </h3>
            
            <div className="grid grid-cols-2 gap-y-4 gap-x-8">
              <div className="space-y-1">
                <p className="text-[11px] font-bold text-gray-400 uppercase">{currentLanguage === 'ko' ? '예약자' : 'Người đặt'}</p>
                <p className="text-sm font-bold text-gray-900">{booking.guestName}</p>
              </div>
              
              <div className="space-y-1">
                <p className="text-[11px] font-bold text-gray-400 uppercase">{currentLanguage === 'ko' ? '연락처' : 'Số điện thoại'}</p>
                <p className="text-sm font-bold text-gray-900">{booking.guestPhone}</p>
              </div>
              
              <div className="space-y-1">
                <p className="text-[11px] font-bold text-gray-400 uppercase">{currentLanguage === 'ko' ? '이용 인원' : 'Số khách'}</p>
              ...
              <div className="space-y-1">
                <p className="text-[11px] font-bold text-gray-400 uppercase">{currentLanguage === 'ko' ? '숙박 기간' : 'Thời gian'}</p>
                <p className="text-sm font-bold text-gray-900">{booking.nights}{currentLanguage === 'ko' ? '박' : ' đêm'}</p>
              </div>
            </div>

            {booking.guestMessage && (
              <div className="bg-blue-50/50 rounded-xl p-3.5 space-y-1.5 border border-blue-50">
                <p className="text-[11px] font-bold text-blue-400 uppercase">{currentLanguage === 'ko' ? '요청사항' : 'Yêu cầu đặc biệt'}</p>
            ...
            <div className="pt-4 border-t border-gray-100 flex items-center justify-between">
              <p className="text-sm font-bold text-gray-900">{currentLanguage === 'ko' ? '총 결제 금액' : 'Tổng số tiền'}</p>
              <div className="text-right">
                ...
                <p className="text-[10px] font-bold text-gray-400 mt-1 uppercase">
                  {booking.paymentStatus === 'paid' ? (
                    currentLanguage === 'ko' ? '결제 완료' : 'Đã thanh toán'
                  ) : (
                    currentLanguage === 'ko' ? '결제 대기' : 'Chờ thanh toán'
                  )}
                </p>
              </div>
            </div>
          ...
          <div className="space-y-3 pt-4 pb-10">
            ...
            >
              ...
              {currentLanguage === 'ko' ? '예약 내역 확인' : 'Xem lịch sử đặt phòng'}
            </button>
          </div>
        </div>
